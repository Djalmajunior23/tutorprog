(() => {
  ...
  function renderTrilhas() {
    ...
    el.innerHTML = TRILHAS.map(t => {
      ...
      return `<article class="card">
        ...
        <small>${escapeHtml(t.dificuldade)} • ${t.xp} XP</small>
        <p>Status: ${done ? '✅ concluída' : '🔄 pendente'}</p>
        <button class="btn" data-trilha="${escapeHtml(t.id)}">${done ? 'Revisar' : 'Iniciar'}</button>
      </article>`;
    }).join('');

    el.querySelectorAll('button[data-trilha]').forEach(btn => btn.addEventListener('click', () => {
    ...
    el.innerHTML = MODULOS.map(m => {
      ...
      return `<article class="card">
        ...
        <details>
          ...
          <code><strong>C#</strong>\n${escapeHtml(m.comparacao?.csharp || '-')}\n\n<strong>Java</strong>\n${escapeHtml(m.comparacao?.java || '-')}\n\n<strong>JavaScript</strong>\n${escapeHtml(m.comparacao?.javascript || '-')}\n\n<strong>Python</strong>\n${escapeHtml(m.comparacao?.python || '-')}</code>
        </details>
        <p><em>Atividade:</em> ${escapeHtml(m.atividade || 'Pratique com um exemplo próprio.')}</p>
        <button class="btn" data-mod="${escapeHtml(m.id)}">${done ? 'Concluído ✅' : 'Concluir (+50 XP)'}</button>
      </article>`;
    }).join('');

    el.querySelectorAll('button[data-mod]').forEach(btn => btn.addEventListener('click', () => {
      const modId = btn.dataset.mod;
      if (modId && !state.modulosConcluidos.includes(modId)) {
        state.modulosConcluidos.push(modId);
        grantXP(50);
        if (state.modulosConcluidos.length === MODULOS.length) grantXP(500);
        persist();
      }
    }));
  }
  ...
  function renderDesafios() {
    ...
    el.innerHTML = DESAFIOS.map(d => `<article class="card"><h4>${escapeHtml(d.titulo)}</h4><p>${escapeHtml(d.comando)}</p><small>${escapeHtml(d.nivel)} • ${d.xp} XP</small><button class="btn" data-load="${escapeHtml(d.id)}">Carregar</button></article>`).join('');

    el.querySelectorAll('button[data-load]').forEach(btn => btn.addEventListener('click', () => {
  ...
  function renderGamificacao() {
    ...
    state.conquistas = CONQUISTAS.filter(c => c.check(state)).map(c => c.id);
    const lc = $('#lista-conquistas');
    const lm = $('#lista-missoes');

    if (lc) lc.innerHTML = CONQUISTAS.map(c => `<article class="card ${state.conquistas.includes(c.id) ? 'ok' : ''}">${escapeHtml(c.titulo)}</article>`).join('');
    if (lm) lm.innerHTML = MISSOES.map(m => `<article class="card ${m.check(state) ? 'ok' : ''}">${escapeHtml(m.titulo)}</article>`).join('');

    const next = $('#proxima-missao');
    if (next) next.textContent = MISSOES.find(m => !m.check(state))?.titulo || 'Todas concluídas!';
  }

  function renderRanking() {
    const el = $('#lista-ranking');
    if (!el || !window.Ranking) return;
    el.innerHTML = Ranking.get().map(r => `<li>${escapeHtml(r.nome)} — ${Number(r.xp) || 0} XP</li>`).join('');
  }
  ...
  function render() {
    ...
    if (mods) mods.textContent = `${state.modulosConcluidos.length} (${progressoTexto(window.MODULOS?.length || 0, state.modulosConcluidos.length)})`;
  ...
  on('#btn-verificar', 'click', () => {
    ...
    const d = DESAFIOS.find(x => (x.criterios || []).every(c => txt.includes(String(c).toUpperCase())));
  ...
  on('#btn-limpar', 'click', () => { if (editor) editor.value = ''; });
  on('#btn-copiar', 'click', () => { if (editor) navigator.clipboard?.writeText(editor.value); });
  on('#btn-salvar', 'click', () => { if (editor) localStorage.setItem('portalEditor', editor.value); });
  on('#btn-restaurar', 'click', () => { if (editor) editor.value = localStorage.getItem('portalEditor') || ''; });
  ...
  if (sel && window.EXEMPLOS) {
    ...
    on('#btn-carregar-exemplo', 'click', () => { if (editor) editor.value = EXEMPLOS[sel.value] || ''; });
  }

  on('#btn-ranking-salvar', 'click', () => { Ranking.update(state.perfil.nome, state.xp); renderRanking(); });
  on('#btn-ranking-limpar', 'click', () => { Ranking.clear(); renderRanking(); });
  on('#btn-exportar', 'click', () => Professor.exportar(state));
  on('#btn-copiar-desafios', 'click', () => Professor.copiarDesafios());
  on('#btn-resetar', 'click', () => { localStorage.removeItem(Portal.KEY); location.reload(); });
