  ...
  function executeLines(lines, vars, out) {
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^ESCREVA\(/i.test(l)) {
        ...
        out.push(String(evalExpr(inside, vars)));
        continue;
      }
      ...
      if (/^SE /i.test(l)) {
        const m = l.match(/^SE\s+(.*)\s+ENTAO$/i);
        if (!m) throw Error(`Erro de sintaxe na linha ${i + 1}`);
        if (!evalExpr(m[1], vars)) {
          while (i < lines.length && !/^SENAO$|^FIM_SE$/i.test(lines[i])) i++;
        }
        continue;
      }
      ...
      if (/^PARA /i.test(l)) {
        const m = l.match(/^PARA\s+(\w+)\s+DE\s+(.+)\s+ATE\s+(.+)\s+FACA$/i);
      ...
      if (/^ENQUANTO /i.test(l)) {
        const m = l.match(/^ENQUANTO\s+(.+)\s+FACA$/i);
        if (!m) throw Error(`ENQUANTO inválido na linha ${i + 1}`);
        const body = [];
        i++;
        while (i < lines.length && !/^FIM_ENQUANTO$/i.test(lines[i])) body.push(lines[i++]);
        let guard = 0;
        while (evalExpr(m[1], vars) && guard < 200) {
          guard++;
          executeLines(body, vars, out);
        }
      }
